...
class CircuitAnalysisActivity : AppCompatActivity() {
    ...
    private fun parseComplex(input: String): Complex {
        ...
        // Handle special pure imaginary cases
        if (cleaned == "j" || cleaned == "+j") return Complex(0.0, 1.0)
        if (cleaned == "-j") return Complex(0.0, -1.0)

        // If no '+' and no '-' after position 0, it's either a pure real or pure imaginary
        if (!cleaned.contains("+") && cleaned.indexOf('-', startIndex = 1) == -1) {
            return when {
                cleaned.endsWith("j") -> Complex(0.0, cleaned.removeSuffix("j").toDouble())
                cleaned.startsWith("j") -> Complex(0.0, cleaned.removePrefix("j").toDouble())
                else -> Complex(cleaned.toDouble(), 0.0)
            }
        }

        // Handle full complex format like "3+4j", "-3-j5", etc.
        ...
        val real = if (realRaw.isNullOrBlank()) 0.0 else realRaw.toDouble()
                ...
                for (i in 0 until n) {
                    ...
                    for (j in 0 until n) {
                        ...
                        if (coeff.re != 0.0 || coeff.im != 0.0) {
                            val formatted = coeff.toFormattedString(decimalPlaces)
                            terms.add("(${formatted})*I${j + 1}")
                        }
                    }
                    if (terms.isNotEmpty()) {
                        append(terms.joinToString(" + "))
                        append(" = ${b[i].toFormattedString(decimalPlaces)} V\n")
                    }
                }
            }.trim()
